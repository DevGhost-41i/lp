      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          <button
            ...
            className="flex items-center space-x-2 bg-[#1E1E1E] hover:bg-[#2C2C2C] text-white px-4 py-2 rounded-lg transition-all hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            onClick={onAddUser}
            className="flex items-center space-x-2 bg-[#48a77f] hover:bg-[#3d9166] text-white px-4 py-2 rounded-lg transition-all shadow-lg shadow-[#48a77f]/20 hover:shadow-[#48a77f]/40"
          >
        ...
        {users.length === 0 && pendingInvitations.length === 0 ? (
          ...
          <>
            {pendingInvitations.map((invitation) => (
              <div
                key={invitation.id}
                className="glass-card rounded-lg p-6 border-l-4 border-l-yellow-500 hover:shadow-lg hover:shadow-yellow-500/10 transition-all duration-300"
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-semibold border ${invitation.role === 'admin'
            ...
            {users.map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className={`glass-card rounded-lg p-6 border-l-4 transition-all duration-300 hover:shadow-lg ${isSuperAdmin
                      ? 'border-l-yellow-500 hover:shadow-yellow-500/10'
                      : isAdmin
                        ? 'border-l-blue-500 hover:shadow-blue-500/10'
                        : 'border-l-[#48a77f] hover:shadow-[#48a77f]/10'
                    }`}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className={`p-3 rounded-lg ${isSuperAdmin ? 'bg-yellow-500' : isAdmin ? 'bg-blue-500' : 'bg-[#48a77f]'
                      }`}>
                      {isSuperAdmin ? (
