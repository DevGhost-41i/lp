  ...
  if (!mcmData) {
    return (
      <div className="glass-card rounded-xl p-6 flex flex-col items-center justify-center text-center h-64">
        ...
        <h3 className="text-sm font-semibold text-gray-300 mb-1">MCM Program Insights</h3>
    ...
    <div className="space-y-4">
      ...
      <div className="glass-card rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Net Revenue Split</p>
          <span className="text-xs font-medium text-[#48a77f] bg-[#48a77f]/10 px-2 py-0.5 rounded">
            {((mcmData.total_mcm_deducted / mcmData.total_revenue) * 100).toFixed(1)}% to MCM
          </span>
        </div>
        <div className="w-full bg-[#2C2C2C] rounded-full h-2.5 overflow-hidden shadow-inner">
          <div
            className="h-full bg-gradient-to-r from-[#48a77f] to-[#5BBF94] transition-all duration-1000 ease-out relative"
            style={{
        ...
        <div className="flex justify-between mt-3 text-xs">
          <div className="flex items-center gap-1.5 text-gray-400">
            <div className="w-2 h-2 rounded-full bg-[#48a77f]" />
            <span>MCM: <span className="text-white font-medium">${mcmData.total_mcm_deducted.toFixed(2)}</span></span>
          </div>
          <div className="flex items-center gap-1.5 text-gray-400">
            <div className="w-2 h-2 rounded-full bg-[#2C2C2C] border border-gray-600" />
            <span>Publishers: <span className="text-white font-medium">${mcmData.publisher_net_revenue.toFixed(2)}</span></span>
          </div>
        </div>
      </div>
    </div>
