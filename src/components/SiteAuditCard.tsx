...
import { Activity, CheckCircle, XCircle, AlertCircle, RefreshCw, Info } from 'lucide-react';
...
export function SiteAuditCard({ audit, publisherName, domain, onAuditTriggered }: SiteAuditCardProps) {
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Last scanned: {siteAuditService.formatDate(audit.scanned_at || audit.completed_at || audit.created_at || null)}
            </p>
          </div>
          <button
            onClick={handleTriggerAudit}
        ...
        {!isInProgress && !isFailed && (
          <>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center">
                  <div className="flex-1 bg-[#1E1E1E] rounded-full h-3 mr-3">
                    <div
                      className={`h-3 rounded-full transition-all ${riskInfo.level === 'low' ? 'bg-[#48a77f]' :
            ...
            {!isDataPending && aiReport && aiReport.interpretation && (
              <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <div className="text-xs font-semibold text-blue-300 mb-2">AI Report</div>
                <div className="space-y-1 text-xs">
                  {aiReport.interpretation?.categorization && (
                    <>
                      <div className="flex justify-between text-gray-300">
                        <span>Category:</span>
                        <span className="text-blue-300 font-medium">{aiReport.interpretation.categorization.primaryCategory || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between text-gray-300">
                        <span>Confidence:</span>
                        <span className="text-blue-300 font-medium">{Math.round((aiReport.interpretation.categorization.confidence || 0) * 100)}%</span>
                      </div>
                    </>
                  )}
                  {((aiReport.interpretation?.parsedFindings?.modules && Object.keys(aiReport.interpretation.parsedFindings.modules).length > 0) ||
                    (aiReport.modules && aiReport.modules.length > 0)) && (
                      <div className="flex justify-between text-gray-300">
                        <span>Modules:</span>
                        <span className="text-blue-300 font-medium">
                          {aiReport.interpretation?.parsedFindings?.modules ? Object.keys(aiReport.interpretation.parsedFindings.modules).length : (aiReport.modules?.length || 0)}
                        </span>
                      </div>
                    )}
                </div>
