...
import { CheckCircle, XCircle, Clock, MoreVertical, Globe, ExternalLink, ChevronUp, ChevronDown, History } from 'lucide-react';
    ...
    return (
      <div className="flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-8 flex-shrink-0">
          <div className="flex gap-2 bg-[#1E1E1E] p-1 rounded-lg w-fit animate-pulse">
            <div className="h-10 w-48 bg-[#2C2C2C] rounded-md"></div>
            <div className="h-10 w-40 bg-[#2C2C2C] rounded-md"></div>
          </div>
        </div>

        {/* Table Skeleton */}
        <div className="mt-6 mx-6 bg-[#161616] rounded-lg border border-[#2C2C2C] shadow-lg flex flex-col overflow-hidden">
          <div className="px-6 py-4 border-b border-[#2C2C2C] animate-pulse">
            <div className="h-6 bg-[#2C2C2C] rounded w-48"></div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead className="bg-[#0E0E0E] border-b border-[#2C2C2C]">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wide">Publisher Name</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wide">Network Code</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wide">Email</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wide">Site</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wide">Partner</th>
                  <th className="px-6 py-4 text-center text-xs font-medium text-gray-400 uppercase tracking-wide">MFA Score</th>
                  <th className="px-6 py-4 text-center text-xs font-medium text-gray-400 uppercase tracking-wide">Actions</th>
                </tr>
              </thead>
                  ...
                  <tr key={i} className="animate-pulse">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-[#2C2C2C] rounded-lg"></div>
                        <div className="h-4 bg-[#2C2C2C] rounded w-32"></div>
                      </div>
                    </td>
                    <td className="px-6 py-4"><div className="h-4 bg-[#2C2C2C] rounded w-24"></div></td>
                    <td className="px-6 py-4"><div className="h-4 bg-[#2C2C2C] rounded w-40"></div></td>
                    <td className="px-6 py-4"><div className="h-4 bg-[#2C2C2C] rounded w-32"></div></td>
                    <td className="px-6 py-4"><div className="h-4 bg-[#2C2C2C] rounded w-28"></div></td>
                    <td className="px-6 py-4 text-center"><div className="h-4 bg-[#2C2C2C] rounded w-8 mx-auto"></div></td>
                    <td className="px-6 py-4 text-center"><div className="h-8 w-8 bg-[#2C2C2C] rounded-md mx-auto"></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="px-6 py-8 flex-shrink-0">
        ...
        <div className="flex gap-2 bg-[#1E1E1E] p-1 rounded-lg w-fit">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'pending'
          ...
          >
            ...
            {pendingPublishers.length > 0 && (
              <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${activeTab === 'pending' ? 'bg-white/20 text-white' : 'bg-yellow-600 text-white'
                }`}>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'activity'
                ? 'bg-[#48a77f] text-white'
                : 'text-gray-400 hover:text-white'
              }`}
          >
            ...
            {approvalLogs.length > 0 && (
              <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${activeTab === 'activity' ? 'bg-white/20 text-white' : 'bg-blue-600 text-white'
                }`}>
                {approvalLogs.length}
              </span>
            )}
                    ...
                    {pendingPublishers.map((publisher) => (
                      <tr key={publisher.id} className="hover:bg-[#1E1E1E] transition-colors duration-150 border-b border-[#2C2C2C]">
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className="text-white font-semibold text-sm">{(publisher as any).mfa_score || 0}</span>
                        </td>
                      ...
                      <div key={log.id} className="bg-[#0A0A0A] rounded-lg p-4 border border-[#2C2C2C] hover:border-[#48a77f]/50 transition-colors">
                        <div className="flex items-start sm:items-center justify-between gap-4">
                          <div className="flex items-start space-x-3 flex-1 min-w-0">
                            <div className={`w-3 h-3 rounded-full flex-shrink-0 mt-1.5 ${log.action === 'status_changed' && log.new_status === 'approved'
                              ? 'bg-[#48a77f]'
              ...
              <div className="p-6">
                ...
                <div className="flex space-x-3">
                  <button
                    ...
                    className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors text-white ${pendingAction === 'approve'
